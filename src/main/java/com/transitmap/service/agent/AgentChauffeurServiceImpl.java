...
public class AgentChauffeurServiceImpl implements AgentChauffeurService {
    ...
    public ChauffeurDto creerChauffeur(ChauffeurDto dto, String usernameAgent) {
        ...
        Role roleChauffeur = roleRepository.findByName("CHAUFFEUR")
                .orElseThrow(() -> new ResourceNotFoundException("Rôle CHAUFFEUR introuvable"));
    ...
    @Override
    @Transactional
    public ChauffeurDto modifierChauffeur(Long id, ChauffeurDto dto, String usernameAgent) {
    ...
    @Override
    @Transactional
    public ChauffeurDto assignerLigne(Long chauffeurId, Long ligneId, String usernameAgent) {
    ...
    public List<ChauffeurDto> trouverParAgent(String usernameAgent) {
        User agent = trouverAgent(usernameAgent);
        return chauffeurRepository.findByAgent(agent).stream().map(this::mapper).toList();
    }
    ...
    private Chauffeur trouverChauffeurDeLAgent(Long id, String usernameAgent) {
        ...
        if (!chauffeur.getAgent().getUsername().equals(usernameAgent)) {
            throw new RuntimeException("Accès non autorisé à ce chauffeur");
        }
        return chauffeur;
    }
